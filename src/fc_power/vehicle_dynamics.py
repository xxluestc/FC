...
class VehicleParams:
    mass_kg: float = 16400.0
    drag_coefficient: float = 0.62
    frontal_area_m2: float = 7.6
    rolling_resistance: float = 0.015
    rotational_mass_factor: float = 1.08
...
def force_power(speed_mps, acceleration_mps2, p: VehicleParams = VehicleParams()):
    ...
    a = np.asarray(acceleration_mps2, float)
    alpha = p.road_grade_rad
    f_air = 0.5 * p.air_density_kg_m3 * p.drag_coefficient * p.frontal_area_m2 * v**2
    ...
    f_trac = f_air + f_roll + f_grade + f_acc
    p_wheel = f_trac * v / 1000
    return {
        "f_air_n": f_air,
        "f_roll_n": f_roll,
        "f_grade_n": f_grade,

...
| 预测 | `prediction/` | 分预测域直接功率模型、模型族比较和制动感知分支 |
| 分配 | `power_allocation/` | 瞬时与滚动优化、SOC和功率约束 |
| 评价 | `evaluation/` | MAE/RMSE、百分比指标和科研图 |

当前控制输入采用 `state_direct_power`：历史30秒车速、加速度、功率、SOC、工况和停车状态直接预测功率。H=1/3/5/10分别训练，比较ExtraTrees、HistGradientBoosting、XGBoost和brake-aware ExtraTrees；模型只按验证集选择。车辆动力学功率仍作为独立校准基线，不与直接功率预测混称。
...
- XGBoost由验证集选为H=1/3/5/10的默认模型；测试逐秒NRMSE为2.38%/3.40%/4.96%/7.53%。
- H=10未达到5%门槛；默认控制时域仍为H=5，远期预测采用指数置信衰减。

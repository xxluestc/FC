...
> **最新纠错（2026-07-14）：** 简化随机负载实验已作废。继续工作先读
...
- `git diff --check`通过；
- Fig.1--26均为PNG；本轮新增Fig.23--26并完成目视检查；
...
同一`G:`归档的2025-06至2026-06另外构成独立跨月功率验证集：13个月各分前/中/后三个时间层，
每层选一个30分钟连续块，共39块、70,200个1秒点。它不与五月七天链拼接，每块重置健康和控制
状态；只验证冻结方法在跨月真实功率下的可执行性与健康均衡方向，不解释为连续13个月老化。
...
慢层当前候选是24小时`Guarded Blend`。适用域内使用序统计量`Blend 0.50`：对每个有序双堆
分配同时计算投影最大损伤和第二大损伤，各取0.5权重；当初始损伤最大堆与估计退化速率最大堆
身份错配，或健康没有区分时，回退固定双堆。它只读取当前健康、异质性估计和冻结开发暴露均值，
不读取未来负载；`health-greedy`与Expected-max保留为首边界基线。
...
- 41场景×20种子参数矩阵中，原始Blend在强GP随机效应错配场景第二边界平均`-47.20 h`且
  95%区间低于0；Guarded Blend无负CI场景，最差均值`-0.50 h`且区间跨0；
- 异质性跨月验证中，参考场景首/第二边界为`+71.07/+4.08 h`；强对齐场景为
  `+50.77/+1.16 h`且第二边界区间跨0；强错配场景260/260组合精确回退固定基线；
- 两阶段RUL、continuity阈值和局部双边界安全投影均已否决。后者在两个异质性场景仍有第二
  边界负CI，不能替代门控；
- 异质性因子来自文献随机效应压力分布的归一化分位数，不是21UBE0022车辆或真实三堆拟合值。
...
冻结40 kW方法在2025-06至2026-06共39个独立30分钟块上完成234例、421,200个闭环步，零硬约束
违规，最大跟踪误差5.496 kW。414个高于40 kW的点没有截断，最高41.164 kW。基础70,200步中
35,117步为不超过10秒遥测段内插值，任何大于10秒的缺口均未跨越。

以13个日历月为主统计单位，health-greedy相对固定双堆的终端最大退化变化均值为
-0.003938848个百分点，月级BCa 95%区间[-0.004063830, -0.003848424]，13/13个月改善。
同时总期望退化增加0.000284514个百分点，跟踪MAE降低0.0494 kW，氢耗强度降低0.1147 g/kWh。
这说明方法稳定改善堆间健康失衡，但不构成所有退化指标上的Pareto占优。详见Fig.20和
...
- `data/results/fc_only_external_monthly_blocks/`：13个月39个独立功率块及源文件哈希；
- `data/results/fc_only_external_monthly_replay/`：跨月冻结回放、月级统计和Fig.20；
...
- `data/results/fc_only_n_plus_one_parameter_robustness/`：41场景参数矩阵与Fig.23；
- `data/results/fc_only_two_stage_continuity_screen/`：两阶段连续性负结果与Fig.24；
- `data/results/fc_only_protected_blend_screen/`：局部安全投影负结果与Fig.25；
- `data/results/fc_only_guarded_blend_cross_month/`：三类异质性跨月验证与Fig.26。
...
python scripts/61_audit_n_plus_one_parameter_robustness.py --seeds 20 --jobs 20
python scripts/62_screen_two_stage_continuity.py --seeds 20 --jobs 12
python scripts/62_screen_two_stage_continuity.py --seeds 20 --jobs 20 `
  --focused-protected --out-dir data/results/fc_only_protected_blend_screen
python scripts/63_validate_guarded_blend_cross_month.py --seeds 20 --jobs 20
```
...
- 不把跨月功率块写成同一物理电堆连续13个月SOH/寿命验证；每块健康状态均重置。
- 不称Guarded Blend在未知异质性下普适最优；部署前必须得到各堆相对退化速率估计。
...
1. 40 kW链、N+1第二边界、41场景鲁棒性和Guarded Blend跨月验证均已完成。
2. 保持动作驱动预测态为主健康闭环；不要把混杂电压或瞬态突降硬转成`D`。
3. 下一阶段优先建立可部署的相对退化速率observer/检测口径，再检验物理失效阈值和维护成本。

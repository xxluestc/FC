...
留出峰值反算的事后严格容量下界，不能回填标定。独立全年归档进一步确认40 kW是长期重复出现的
正常控制档，因此最终链把它作为经验功率归一化参考；它仍不是已确认的铭牌额定净功率。
...
规划阶段始终使用当前posterior和动作条件均值，不读取未来观测。动作实际执行后，observer先传播
prediction方差

$$
P^-_{j,t+1}=P^+_{j,t}+E[\Delta D^{\rm cont}_{j,t}]b_j+q_j\Delta t,
$$

其中离散事件项在当前模型中保持确定性。若同一执行步末端存在已校准退化代理观测$y_{j,t+1}$，
再执行标量更新

$$
K=\frac{P^-}{P^-+R},\qquad
\tilde D^+=D^-+K(y-D^-),\qquad
D^+=\max(D_{j,t},\tilde D^+).
$$

最后一项是累计退化单调投影，投影事件必须记录。当前`ObservedHealthExecutionLoop.execute`已实现
该因果边界，且无观测时posterior严格等于prediction。现有合成实验只输入直接退化代理，用于验证
软件时序和模型漂移校正；未建立21UBE0022原始电压/电流到$D$或$\theta$的测量模型，因此主实验
仍不启用观测校正。

实现：`src/fc_power/health/observer.py`、
`src/fc_power/world_model/observed_health.py`；接口说明见
`docs/HEALTH_OBSERVER_INTERFACE.md`。
...
自身目标一致性另在四种冻结开发暴露与三种健康身份的12个决策点上审计。每点枚举六个有序
分配并同时计算Expected-max与Gamma-CVaR遗憾；该审计不读取未来轨迹，也不把“在定义目标上
最优”重复解释为长期性能优势。
...
归一化参考敏感性保持所有控制器参数和留出样本不变，仅将单堆归一化分母由冻结30 kW改为
事后35/40 kW，并分别重放全部144案例。该分析用于诊断C8对截峰的依赖，不用于选择或确认物理
额定功率。

冻结5.5 kW跟踪容差通过最大误差案例定向扫描解释：在segment 42、最老堆0、固定分配`(1,0)`
中，4.90 kW失败而4.95 kW成功。该扫描保持动作网格、健康和驻留闭环不变，但不扩展为所有案例
的容差敏感性主张。

## 8. 当前不可声称内容

- 不称30 kW或40 kW为铭牌物理额定功率；40 kW只称全年实车支持的经验运行参考，30 kW只作冻结敏感性；

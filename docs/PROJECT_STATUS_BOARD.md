...
|---|---|---|---|---|
| G0 | 仓库、数据与交接可复现 | 已完成 | 数据物化、94项测试、baseline | 持续回归 |
...
| G7 | 双时间尺度调度与机制消融 | 留出选择验证通过 | 18案例oracle集合命中100%、零遗憾 | 全量连续快层回放 |
...
| 7 | 留出全量连续快层回放 | 当前主任务 | segment 22-45不抽窗；状态连续、零违规、容差内 |
| 8 | 真实健康观测变量审计 | 等待输入 | 明确可观测量、时间同步和噪声模型 |

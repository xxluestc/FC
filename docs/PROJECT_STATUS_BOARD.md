...
|---|---|---|---|---|
| G0 | 仓库、数据与交接可复现 | 已完成 | 数据物化、93项测试、baseline | 持续回归 |
...
| G7 | 双时间尺度调度与机制消融 | 主方法开发冻结 | 120个快层模板；强基线与11组稳健性 | 真实留出验证 |
...
| 6 | 冻结方法真实留出验证 | 当前主任务 | 不调参；连续快层回放与慢层选择遗憾 |
| 7 | 真实健康观测变量审计 | 等待输入 | 明确可观测量、时间同步和噪声模型 |

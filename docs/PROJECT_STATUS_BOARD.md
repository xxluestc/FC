...
| 8 | 负载归一化与N+1容量审计 | 当前物理确认项 | 30 kW包络、36.756 kW事后下界、40 kW候选分开报告 |
| 9 | 真实健康观测变量审计 | 等待输入 | 明确可观测量、时间同步和噪声模型 |
...
| 完整真实留出 | 冻结health-greedy方向有效且全例可执行 | 段内连续、段间不跨未知缺口；高于30 kW样本被截峰 |

...
|---|---|---|---|---|
| G0 | 仓库、数据与交接可复现 | 已完成 | 数据物化、108项测试、baseline | 持续回归 |
...
| G7 | 双时间尺度调度与机制消融 | N+1第二边界目标完成开发与跨月验证 | 200种子稳健性；13个月×20种子；Blend 0.50同时保护首/第二边界 | 冻结候选，继续物理阈值与异质性验证 |
...
| 慢层主方法 | 24小时序统计量Blend 0.50候选 | 兼顾最大损伤与第二大损伤；开发200种子和13个月外部负载均值区间均为正 |

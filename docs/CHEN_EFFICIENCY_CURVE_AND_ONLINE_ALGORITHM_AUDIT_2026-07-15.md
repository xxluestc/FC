...
留出集 online hysteresis 相对 instantaneous 平均减少 `46.59%` 的电堆状态变化，氢耗增加 `0.190%`；在切换权重 `2x` 时综合目标改善 `0.144%`。当前结论是可重复的效率-切换 Pareto，不是寿命或真实退化收益。

原计划中的 Borodin `A_f` 暂不实现。当前更优先的科学缺口是 N+1 故障退出是否真的能够在线重构；在该闭环跑通前继续增加算法名称不会提高结果可信度。

...
| `fig34_lzw_health_progress` | 刘占伟6104个稳定事件的`theta=[i0,ih,R]`定向归一化、单调投影与`h -> theta(h)`重构诊断。`h=1`只表示观测轨迹末端，不是SOH、EOL或失效阈值。 |
| `fig35_relative_health_adaptive_replay` | 系数无关的相对健康自适应回放。左上显示需要双堆供电的实车映射需求及分配，右上比较健康保留与在线性能自适应的相对健康写回，左下比较29个后期开发块的终端健康极差，右下给出健康权重与跟踪误差的折中。总健康预算对所有策略固定相同；该图只证明健康自适应链路，不证明物理退化率或寿命延长。 |
